...
DEFAULT_ORDER = ["openai", "ollama", "nvidia", "groq", "gemini"]
...
TIER_TO_ORDER = {
    "TINY":  ["openai", "ollama", "nvidia", "groq", "gemini"],
    "LARGE": ["openai", "ollama", "nvidia", "groq", "gemini"],
}
...
@app.get("/v1/live-demo/stream")
async def live_demo_stream():
    """Stream live assignment queries (real gateway path) into dashboard."""
    script = ROOT.parent / "agent6.py"
    ...
    async def gen():
        try:
            ...
            db.log_call(
                ...
                error=None if code == 0 else f"live demo exited with {code}",
                call_role="done",
                tool_dialect="live",
            )
