...
```powershell
curl.exe -s http://localhost:8101/v1/routers | py -3 -m json.tool
```
...
```powershell
cd C:\path\to\llm_gatewayV3
.\run.ps1                # creates .venv on first run, then starts on port 8101
...
|---|---|---|
| **TINY**  | < 1,000 | ollama → openai → nvidia → groq → gemini |
| **LARGE** | 1,000 – 8,000 | ollama → openai → nvidia → groq → gemini |
...
Failover order (worker pool) is configurable via `LLM_ORDER` env. Default: `ollama,openai,nvidia,groq,gemini`.
...
LLM_ORDER=ollama,openai,nvidia,groq,gemini
...
```bash
xdg-open http://localhost:8101  # Linux
# macOS: open http://localhost:8101
```

Windows PowerShell:

```powershell
Start-Process http://localhost:8101
```
...
- [run.sh](run.sh) — Linux/macOS launcher (venv setup + start on port 8101)
- [run.ps1](run.ps1) — Windows PowerShell launcher (venv setup + start on port 8101)
- [run.bat](run.bat) — Windows CMD launcher (venv setup + start on port 8101)
...
```powershell
.\.venv\Scripts\python.exe tests\test_all_providers.py
```
