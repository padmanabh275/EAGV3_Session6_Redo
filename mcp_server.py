...
File tools are sandboxed under ./sandbox/. Run:  py -3 mcp_server.py.py
"""
...
async def _crawl4ai_fetch(url: str, timeout: int = 20) -> dict:
    ...
    os.dup2(2, 1)
    try:
        # Use file-only logger to prevent Rich Unicode console output from
        # corrupting/encoding-failing MCP stdio on Windows.
        logger = AsyncFileLogger(str(Path(__file__).parent / ".crawl4ai" / "crawler.log"))
...
async def fetch_url(url: str, timeout: int = 20) -> dict:
    ...
    try:
        # Bound the entire crawl lifecycle (browser startup + fetch + teardown).
        return await asyncio.wait_for(_crawl4ai_fetch(url, timeout=timeout), timeout=timeout + 10)
    except asyncio.TimeoutError:
        return {
            "status": 504,
            "content_type": "text/plain",
            "length_bytes": 0,
            "text": f"fetch_url timeout after {timeout}s for {url}",
        }
